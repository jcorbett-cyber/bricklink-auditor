...
with st.sidebar:
    ...
    if st.session_state.audit_mode:
        ...
        if st.button(f"🏠  Dashboard",        use_container_width=True): st.session_state.page = "dashboard"; st.rerun()
        if st.button(f"📦  Browse Inventory", use_container_width=True): st.session_state.page = "browse";    st.rerun()
        if st.button(f"📊  Summary",          use_container_width=True): st.session_state.page = "summary";   st.rerun()
        if st.button(f"🗃️  Stockroom",        use_container_width=True): st.session_state.page = "stockroom"; st.rerun()
        if st.button(f"📋  Duplicates",       use_container_width=True): st.session_state.page = "dupes";     st.rerun()
        if st.button(f"🏷️  Price Checker",    use_container_width=True): st.session_state.page = "prices";    st.rerun()
        if st.button(f"📦  Pull Orders",      use_container_width=True): st.session_state.page = "orders";    st.rerun()
        if st.button(f"🔍  Skipped Items",    use_container_width=True): st.session_state.page = "skipped";   st.rerun()
        if st.button(f"📅  Audit History",    use_container_width=True): st.session_state.page = "history";   st.rerun()
